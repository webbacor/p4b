...
class ChapterManager extends Manager{
    ...
    public function getChapters(){
      $db = $this->dbConnect();
      $req = $db->query('SELECT *, DATE_FORMAT(date_chapter, \'%d/%m/%Y\') AS date_chapter_fr FROM chapters ORDER BY date_chapter DESC');
      if ($req === false) {
    ...
    public function getChapter($id){
      $db = $this->dbConnect();
      $req = $db->prepare('SELECT *, DATE_FORMAT(date_chapter, \'%d/%m/%Y\') AS date_chapter_fr FROM chapters WHERE id = ?');
      if ($req === false) {
    ...
    public function editChapter($id, $content, $title) {
      $db = $this->dbConnect();
      $chapter = $db->prepare('UPDATE `chapters` SET `title`=?,`content`=? WHERE id=?');
      $chapter->execute(array($title, $content, $id));
      return $chapter;
    }
